...
class ProductFormVariationsCard extends Component {
	 ...
	 * A user shouldn't loose work just because they locally toggled a card.
	 */
	handleToggle = () => {
		const simpleFields = [ 'dimensions', 'weight', 'regular_price', 'manage_stock', 'stock_quantity', 'backorders' ];
		if ( 'variable' !== this.props.product.type ) {
			this.setProductTypeVariable( simpleFields );
		} else {
			this.setProductTypeSimple( simpleFields );
		}
	}

	setProductTypeVariable( simpleFields ) {
	...
	setProductTypeSimple( simpleFields ) {
		const { product, editProduct } = this.props;
		const productData = { ...product };
		const simpleProduct = this.state.simpleProduct;
